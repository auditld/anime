...
export default function CompletedPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await AnimeAPI.getCompletedAnime(currentPage);
        setData(response);
      } catch (err) {
        console.error("Error loading completed anime:", err);
  ...
  if (data.data.length === 0) {
    return (
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8">Completed Anime</h1>
        <EmptyState title="No completed anime" message="Check back later for new content." />
      </div>
    );
  }
